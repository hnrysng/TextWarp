function StaticSingleFontOnCanvas(typeface, letter, assets){
    ...
    for(let i = 0; i < letter.length; i++){
        ...
        for(let i = 0; i < typeface_path_con.length; i++){
            for(let j = 0; j < typeface_path_con[i].length; j++){
                for(let k = 0; k < typeface_path_con[i][j].length; k++){
                    typeface_path_con[i][j][k].x = (typeface_path_con[i][j][k].x * assets.scale) + updated_width_con[i] + text_alignment;
                    typeface_path_con[i][j][k].y = ((-typeface_path_con[i][j][k].y + typeface.parameters.ascender) * assets.scale) + assets.position.y + pos_y_align[i];
                    typeface_path_con[i][j][k].in_x = (typeface_path_con[i][j][k].in_x * assets.scale) + updated_width_con[i] + text_alignment;
                    typeface_path_con[i][j][k].in_y = ((-typeface_path_con[i][j][k].in_y + typeface.parameters.ascender) * assets.scale) + assets.position.y + pos_y_align[i];
                    typeface_path_con[i][j][k].out_x = (typeface_path_con[i][j][k].out_x * assets.scale) + updated_width_con[i] + text_alignment;
                    typeface_path_con[i][j][k].out_y = ((-typeface_path_con[i][j][k].out_y + typeface.parameters.ascender) * assets.scale) + assets.position.y + pos_y_align[i];
                }
            }
        }
        ...
        for(let i = 0; i < typeface_path_con.length; i++){
            for(let j = 0; j < typeface_path_con[i].length; j++){
                for(let k = 0; k < typeface_path_con[i][j].length; k++){
                    ...
                    if(max_val < typeface_path_con[i][j][k].x){
                        max_val = typeface_path_con[i][j][k].x;
                    }

                    if(max_val < typeface_path_con[i][j][k].in_x){
                        max_val = typeface_path_con[i][j][k].in_x;
                    }

                    if(max_val < typeface_path_con[i][j][k].out_x){
                        max_val = typeface_path_con[i][j][k].out_x;
                    }
                }
            }
        }
        ...
        for(let i = 0; i < typeface_path_con.length; i++){
            for(let j = 0; j < typeface_path_con[i].length; j++){
                for(let k = 0; k < typeface_path_con[i][j].length; k++){
                    ...
                    if(min_val >= typeface_path_con[i][j][k].x){
                        min_val = typeface_path_con[i][j][k].x;
                    }

                    if(min_val >= typeface_path_con[i][j][k].in_x){
                        min_val = typeface_path_con[i][j][k].in_x;
                    }

                    if(min_val >= typeface_path_con[i][j][k].out_x){
                        min_val = typeface_path_con[i][j][k].out_x;
                    }
                }
            }
        }
        ...
        for(let i = 0; i < letter.length; i++){
            for(let j = 0; j < typeface_path_con[i].length; j++){
                    ...
                    ctx.moveTo(typeface_path_con[i][j][0].x, typeface_path_con[i][j][0].y); 

                    for(let k = 0; k < typeface_path_con[i][j].length - 1; k++){
                        ctx.bezierCurveTo(
                            typeface_path_con[i][j][k].out_x, typeface_path_con[i][j][k].out_y,
                            typeface_path_con[i][j][k + 1].in_x, typeface_path_con[i][j][k + 1].in_y,
                            typeface_path_con[i][j][k + 1].x, typeface_path_con[i][j][k + 1].y
                        );
                    }

                    ctx.bezierCurveTo(
                        typeface_path_con[i][j][typeface_path_con[i][j].length - 1].out_x, typeface_path_con[i][j][typeface_path_con[i][j].length - 1].out_y,
                        typeface_path_con[i][j][0].in_x, typeface_path_con[i][j][0].in_y,
                        typeface_path_con[i][j][0].x, typeface_path_con[i][j][0].y
                    );

                    ctx.lineTo(typeface_path_con[i][j][0].x, typeface_path_con[i][j][0].y);
